...
class ABL(nn.Module):
    ...
    def __init__(
        ...
        self.weight_func = (
            lambda w, max_distance=max_clip_dist: torch.clamp(w, max=max_distance)
            / max_distance
        )
    ...
    def logits2boundary(self, logit):
        ...
        edge2 = torch.nn.functional.conv2d(
            kl_combine_bin, dilate_weight, stride=1, padding=1
        )
    ...
    def gt2boundary(self, gt, ignore_label=-1):  # gt NHW
        ...
        gt_ud = gt[:, :, 1:] - gt[:, :, :-1]
        gt_lr = (
            torch.nn.functional.pad(gt_lr, [0, 0, 0, 1, 0, 0], mode="constant", value=0)
            != 0
        )
        gt_ud = (
            torch.nn.functional.pad(gt_ud, [0, 1, 0, 0, 0, 0], mode="constant", value=0)
            != 0
        )
    ...
    def get_direction_gt_predkl(self, pred_dist_map, pred_bound, logits):
        ...
        pred_dist_map_d = torch.nn.functional.pad(
            pred_dist_map, (1, 1, 1, 1, 0, 0), mode="constant", value=max_dis
        )  # NH+2W+2

        logits_d = torch.nn.functional.pad(
            logits, (0, 0, 1, 1, 1, 1, 0, 0), mode="constant"
        )  # N(H+2)(W+2)C
    ...
    def get_dist_maps(self, target):
        ...
        dist_maps = torch.cat(
            [
                self.dist_map_transform(target_detach[i])
                for i in range(target_detach.shape[0])
            ]
        )
        out = -dist_maps
    ...
    def forward(self, logits, target):
        ...
        if ph != h or pw != w:
            logits = F.interpolate(
                input=logits, size=(h, w), mode="bilinear", align_corners=True
            )
        ...
        dist_maps = self.get_dist_maps(
            gt_boundary
        ).cuda()  # <-- it will slow down the training, you can put it to dataloader.
        ...
        direction_gt, direction_pred, weight_ce = self.get_direction_gt_predkl(
            dist_maps, pred_boundary, logits
        )  # NHW,NHW,NCHW
