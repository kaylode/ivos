...
parser.add_argument(
    "--binary", action="store_true", help="Whether to split into binary masks"
)
        ...
        if image_dict["mask"] is not None:
            if args.binary:
                ...
                for label in labels:
                    ...
                    tmp_mask = mask == label
                    os.makedirs(
                        osp.join(target_labelsTr, CLASSNAMES[label]), exist_ok=True
                    )
                    dest_gt_path = osp.join(
                        target_labelsTr, CLASSNAMES[label], train_maskname
                    )
                    df_dict["train"]["image"].append(
                        osp.join("TrainImage", train_filename)
                    )
                    df_dict["train"]["label"].append(
                        osp.join("TrainMask", CLASSNAMES[label], train_maskname)
                    )
                    save_ct_from_npy(
